const crops = ["wheat", "rice", "maize"] as const;
type Crop = (typeof crops)[number];

type CountryId = "usa" | "chn" | "ind" | "bra" | "nga" | "fra";
...
interface HarvestSnapshot {
  year: number;
  values: Record<CountryId, Record<Crop, number>>;
}

interface ProteinSnapshot {
  year: number;
  values: Record<CountryId, number>;
}
...
interface DeepInsightDefinition {
  title: string;
  summary: string;
  detail: string;
}
...
const harvestTimeline: HarvestSnapshot[] = [
  {
    year: 1990,
    values: {
...
const proteinTimeline: ProteinSnapshot[] = [
  {
    year: 1990,
...
const highlightInsights: InsightDefinition[] = [
  {
    year: 1990,
...
const deepInsights: DeepInsightDefinition[] = [
  {
    title: "Feed vs. food",
    summary: "Maize surges faster than population growth",
    detail:
      "In 2020, the atlas countries harvest 833 million tonnes of maize—over triple their 1990 output—driven by feed demand and ethanol policies.",
  },
  {
    title: "Dietary tipping point",
    summary: "China's meat intake now rivals Europe",
    detail:
      "Chinese per capita meat availability hits 75 kg, brushing past France's 86 kg as poultry and pork consumption accelerate in urban centers.",
  },
  {
    title: "Resilience gap",
    summary: "Nigeria's staples diversify slowly",
    detail:
      "Despite doubling rice harvests, Nigeria still imports to satisfy cities—maize gains buffer volatility but highlight infrastructure needs.",
  },
];

const cropNames: Record<Crop, string> = {
  wheat: "Wheat",
  rice: "Rice",
  maize: "Maize",
};

const cropPalette: Record<Crop, string> = {
  wheat: "var(--accent-1)",
  rice: "var(--accent-2)",
  maize: "var(--accent-3)",
};

const countryPalette: Record<CountryId, string> = {
  usa: "#2778c4",
  chn: "#e85d66",
  ind: "#f2a65a",
  bra: "#43b892",
...
const continentContours: Array<[string, Array<[number, number]>]> = [
  [
    "North America",
    [
      [-168, 72],
...
const bubbleElements = new Map<CountryId, { circle: SVGCircleElement; label: SVGTextElement }>();
let bubbleLayer: SVGGElement | null = null;
let labelLayer: SVGGElement | null = null;

function projectPoint(lon: number, lat: number, width: number, height: number): { x: number; y: number } {
  const x = ((lon + 180) / 360) * width;
  const y = ((90 - lat) / 180) * height;
  return { x, y };
}

function buildPath(points: Array<[number, number]>, width: number, height: number): string {
  return points
    .map((coord, index) => {
...
function renderBaseMap(svg: SVGSVGElement): void {
...
function totalForCountry(snapshot: HarvestSnapshot, country: CountryId): number {
  const values = snapshot.values[country];
  return crops.reduce((total, crop) => total + values[crop], 0);
}

function dominantCrop(snapshot: HarvestSnapshot, country: CountryId): Crop {
  const entries = Object.entries(snapshot.values[country]) as Array<[Crop, number]>;
  entries.sort((a, b) => b[1] - a[1]);
  return entries[0][0];
}

function computeGlobalTotals(snapshot: HarvestSnapshot): Record<Crop, number> {
...
function computeMaxProduction(): number {
  return Math.max(
    ...harvestTimeline.flatMap((snapshot) => countries.map((country) => totalForCountry(snapshot, country.id)))
  );
}

function updateBubbles(snapshot: HarvestSnapshot): void {
  if (!bubbleLayer || !labelLayer) {
    return;
  }

  const viewBox = (bubbleLayer.ownerSVGElement as SVGSVGElement).viewBox.baseVal;
  const maxProduction = computeMaxProduction();

  countries.forEach((country) => {
    let circle = bubbleElements.get(country.id)?.circle;
    let label = bubbleElements.get(country.id)?.label;
...
function renderCountryDetail(snapshot: HarvestSnapshot, country: Country): void {
...
function renderDetailPanel(snapshot: HarvestSnapshot): void {
  const details = document.getElementById("map-details");
  if (!details) {
    return;
  }
...
function initTimelineControls(): void {
  ...
  Array.from(ticks.querySelectorAll<HTMLSpanElement>("span")).forEach((tick) => {
    tick.style.cursor = "pointer";
    tick.addEventListener("click", () => {
      const index = Number.parseInt(tick.dataset.index ?? "0", 10);
...
function updateTickHighlight(): void {
  const ticks = Array.from(document.querySelectorAll<HTMLSpanElement>("#timeline-ticks span"));
  ticks.forEach((tick) => {
    tick.style.color = Number.parseInt(tick.dataset.index ?? "0", 10) === activeYearIndex ? "var(--accent-1)" : "";
    tick.style.fontWeight = Number.parseInt(tick.dataset.index ?? "0", 10) === activeYearIndex ? "700" : "500";
  });
}

function renderSnapshotInsights(): void {
  ...
  container.innerHTML = highlightInsights
    .map((insight) => {
      const snapshot = harvestTimeline.find((item) => item.year === insight.year) ?? harvestTimeline[0];
      const totals = computeGlobalTotals(snapshot);
...
function renderStapleChart(): void {
  const svg = document.getElementById("staple-chart") as SVGSVGElement | null;
  if (!svg) {
    return;
  }
  ...
  const totalsByCrop: Record<Crop, number[]> = {
    wheat: [],
    rice: [],
    maize: [],
  };
  ...
  const maxTotal = Math.max(
    ...harvestTimeline.map((snapshot) => Object.values(computeGlobalTotals(snapshot)).reduce((sum, value) => sum + value, 0))
  );
  ...
  const y = (value: number): number => margin.top + (1 - value / maxTotal) * chartHeight;
...
function renderProteinChart(): void {
  const svg = document.getElementById("protein-chart") as SVGSVGElement | null;
  if (!svg) {
    return;
  }
  ...
  const maxY = Math.max(
    ...proteinTimeline.map((snapshot) => Math.max(...Object.values(snapshot.values)))
  );
  ...
  const y = (value: number): number => margin.top + (1 - value / maxY) * chartHeight;
...
function renderDeepInsights(): void {
...
function initThemeToggle(): void {
  ...
  const preferred = (localStorage.getItem("atlas-theme") as "atlas" | "twilight" | null) ?? "atlas";
...
function initScrollAnimations(): void {
  ...
  document.querySelectorAll<HTMLElement>(".fade-in").forEach((element) => observer.observe(element));
}

function initialiseAtlas(): void {
  initThemeToggle();
  renderSnapshotInsights();
  renderDeepInsights();

  const map = document.getElementById("world-map") as SVGSVGElement | null;
  if (map) {
    renderBaseMap(map);
    updateBubbles(harvestTimeline[activeYearIndex]);
    renderDetailPanel(harvestTimeline[activeYearIndex]);
