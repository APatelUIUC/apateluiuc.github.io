...
const spotlights: SpotlightItem[] = [
  {
    ...
    description:
      "I craft delightful experiences that bridge design and technology, from immersive data visualizations to polished product prototypes.",
    icon: "🛠️",
    accent: "--accent-1",
  },
  {
    ...
    description:
      "Empathy drives my process. I partner with teams to shape stories, workshops, and tools that put people first.",
    icon: "💡",
    accent: "--accent-2",
  },
  {
    ...
    description:
      "Whether speaking, writing, or shipping open-source tools, I believe ideas grow stronger when they are shared.",
    icon: "📣",
    accent: "--accent-3",
  },
];
...
const typedTextConfigs: TypedTextConfig[] = [];

function createSpotlightCard(item: SpotlightItem): HTMLDivElement {
...
function createJourneyItem(item: JourneyMilestone): HTMLDivElement {
...
function renderSpotlights(container: HTMLElement): void {
  spotlights.forEach((item) => container.appendChild(createSpotlightCard(item)));
}

function renderJourney(container: HTMLElement): void {
  journey.forEach((item) => container.appendChild(createJourneyItem(item)));
}

function renderSocialLinks(container: HTMLElement): void {
...
function initTypedText(config: TypedTextConfig): void {
...
function registerHoverParallax(): void {
...
function registerCardTilt(): void {
  const cards = Array.from(document.querySelectorAll<HTMLDivElement>(".spotlight-card"));
  cards.forEach((card) => {
...
function restoreTheme(): void {
  const saved = localStorage.getItem("preferred-theme") as ThemeMode | null;
  if (!saved) {
...
function initialize(): void {
  const spotlightContainer = document.querySelector<HTMLElement>("#spotlight-grid");
  const journeyContainer = document.querySelector<HTMLElement>("#journey-timeline");
  const socialContainer = document.querySelector<HTMLElement>("#social-links");
  const highlightElement = document.querySelector<HTMLElement>("[data-highlight]");
